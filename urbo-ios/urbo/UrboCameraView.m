	...
	if ([[AVCaptureDevice devicesWithMediaType:AVMediaTypeVideo] count] > 1)
	{
		...
		if (useFrontCamera)
		{
			...
			newVideoInput = [[AVCaptureDeviceInput alloc]
							 ...
							 error:&error];
			[[Urbo sharedInstance].pexeso initLiveFeed:480 height:640];
		}
		else
		{
			...
			newVideoInput = [[AVCaptureDeviceInput alloc]
							 ...
							 error:&error];
			[[Urbo sharedInstance].pexeso initLiveFeed:720 height:1280];
		}
...
{
	[[Urbo sharedInstance].pexeso pushFrame:sampleBuffer];
}
...
- (void)dealloc
{
	[[Urbo sharedInstance].pexeso stopLiveFeed];
}
